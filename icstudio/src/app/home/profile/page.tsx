...
import { AlertCircle, Camera, CheckIcon, PencilIcon, XIcon, Loader2 } from "lucide-react"
...
const ProfilePage = () => {
  ...
  const [verifyContact, setVerifyContact] = useState<string>("") 
  ...
  useEffect(() => {
    ...
    const fetchUserData = async () => {
      try {
        ...
        if (session?.user) {
          console.log("[前端DEBUG] 使用NextAuth会话中的用户信息:", session.user);
        }
        ...
        console.log("[前端DEBUG] API响应状态:", response.status, response.statusText)
        
        if (!response.ok) {
          const errorData = await response.json()
          console.log("[前端DEBUG] API错误响应:", errorData)
  ...
  const sendVerificationCode = async () => {
    try {
      ...
      const contact = verifyType === "email" ? editedUser?.email : editedUser?.phone
      
      if (!contact) {
        throw new Error(`请先输入${verifyType === "email" ? "邮箱" : "手机号"}`)
      }
          ...
          <Card className="w-full max-w-md mx-auto">
            <CardContent className="pt-6">
              <div className="flex justify-center items-center h-40">
                ...
                <p className="ml-2 text-lg">加载中...</p>
              </div>
            </CardContent>
          </Card>
        </div>
        ...
        <div className="w-full max-w-3xl">
          ...
          <Tabs defaultValue="profile" className="w-full">
            ...
            <TabsContent value="avatar" className="mt-4">
              <Card>
                <CardHeader className="pb-2">
                  <CardTitle className="text-2xl font-bold">头像设置</CardTitle>
                </CardHeader>
                <CardContent className="pt-4">
                  <div className="flex flex-col items-center space-y-6">
                    <div className="relative group">
                      <Avatar className="h-32 w-32 cursor-pointer transition-all duration-300 group-hover:opacity-80">
                        {avatarPreview ? (
                          <AvatarImage src={avatarPreview} alt={user?.name || "用户头像"} />
                        ) : user?.avatar ? (
                          <AvatarImage src={user.avatar} alt={user.name} />
                        ) : (
                          <AvatarFallback className="text-4xl bg-primary text-primary-foreground">
                            {user ? getInitials(user.name) : "用"}
                          </AvatarFallback>
                        )}
                      </Avatar>
                    ...
                    {!avatarPreview && (
                      <div className="text-center text-sm text-muted-foreground">
                        <p>点击头像更换图片</p>
                        <p className="mt-1">支持JPG、PNG、GIF和WEBP格式，文件大小不超过5MB</p>
                      </div>
                    )}
                  </div>
                </CardContent>
        ...
        <DialogContent>
            ...
            <div className="space-y-2">
              <div className="flex justify-between items-center">
                ...
                <div className="flex space-x-2">
                  <Button 
                    variant={verifyType === "email" ? "default" : "outline"}
                    size="sm"
                    onClick={() => setVerifyType("email")}
                    disabled={!editedUser?.email}
                  >
                    邮箱验证
                  </Button>
                  <Button 
                    variant={verifyType === "phone" ? "default" : "outline"}
                    size="sm"
                    onClick={() => setVerifyType("phone")}
                    disabled={!editedUser?.phone}
                  >
                    手机验证
                  </Button>
                </div>
          ...
          <div className="flex justify-end space-x-2">
            ...
            <Button 
              ...
              disabled={verifyLoading || !verifyCode || !password}
            >
              {verifyLoading ? (
                <>
                  <Loader2 className="mr-2 h-4 w-4 animate-spin" />
